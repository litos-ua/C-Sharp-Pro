...
### Завдання DoctorAppointment.
- Створено інтерфейс користувача у консолі.
- DoctorService, PatientService, та AppointmentService — це сервіси для керування даними про лікарів, 
  пацієнтів і зустрічі відповідно. Основний додаток DoctorAppointment створюється з передачею всіх сервісів: 
  doctorService, patientService, і appointmentService. 
- виділений шар, в який просто повинен приймати дані через інтерфейс користувача, а сутності створюються, 
  модифікуються, видаляються в сервісному шарі.
- зроблено базовий інтерфейс IGenericService для усіх севісних інтерфейсів
- методи додавання, модіфікації ...   поки що працюють частково, реалізовано те що виводиться в стартовому 
  меню.
- за допомогою класу PathHelper реалізовано використання відносних замість абсолютних шляхів.
- додані нові типи до Enum.
